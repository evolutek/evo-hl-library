...
class TestVect2DOrientedTransform:
    ...
    def test_full_transform(self):
        ...
        sensor = Vect2DOriented(100, 50, 0)
    ...
    def test_change_referencial_legacy(self):
        ...
        """
        robot = Vect2DOriented(500, 300, math.pi / 2)
        local = Vect2D(100, 0)
...
class TestVect2DOrientedInverse:
    def test_inverse_roundtrip(self):
        pose = Vect2DOriented(100, 200, 0.7)
        point = Vect2D(50, 30)
...
class TestVect2DOrientedCompose:
    def test_compose_translations(self):
        a = Vect2DOriented(10, 20, 0)
        b = Vect2DOriented(5, 3, 0)
        c = a.compose(b)
        assert c == Vect2DOriented(15, 23, 0)

    def test_compose_with_rotation(self):
        """Robot at (100, 0) facing up, sensor at (0, 50) locally."""
        robot = Vect2DOriented(100, 0, math.pi / 2)
        sensor_local = Vect2DOriented(0, 50, 0)
...
class TestVect2DOrientedFromDict:
    def test_full(self):
        p = Vect2DOriented.from_dict({"x": 10, "y": 20, "theta": 0.5})
        assert p == Vect2DOriented(10, 20, 0.5)

    def test_missing_theta(self):
        p = Vect2DOriented.from_dict({"x": 10, "y": 20})
        assert p.theta == 0.0
    ...
    def test_from_config_like_dict(self):
        ...
        pose = Vect2DOriented.from_dict(config["pose"])
        assert pose == Vect2DOriented(0, 85, 0)


class TestVect2DOrientedToDict:
    def test_basic(self):
        assert Vect2DOriented(10, 20, 0.5).to_dict() == {"x": 10.0, "y": 20.0, "theta": 0.5}

    def test_round_trip(self):
        p = Vect2DOriented(10, 20, 0.5)
        assert Vect2DOriented.from_dict(p.to_dict()) == p


class TestVect2DOrientedConversion:
    def test_to_3d_oriented(self):
        p = Vect2DOriented(10, 20, 0.5)
        p3 = p.to_3d_oriented(z=100)
        assert p3 == Vect3DOriented(10, 20, 100, yaw=0.5)


class TestVect2DOrientedEquality:
    def test_equal(self):
        assert Vect2DOriented(1, 2, 0.5) == Vect2DOriented(1, 2, 0.5)

    def test_not_equal(self):
        assert Vect2DOriented(1, 2, 0.5) != Vect2DOriented(1, 2, 0.6)

    def test_not_equal_to_other_type(self):
        assert Vect2DOriented(1, 2, 0) != Vect2D(1, 2)


class TestVect2DOrientedRepr:
    def test_repr(self):
        assert repr(Vect2DOriented(10, 20, 0.5)) == "Vect2DOriented(x=10.0, y=20.0, theta=0.5)"
...
class TestVect3DOrientedConstruction:
    def test_basic(self):
        p = Vect3DOriented(1, 2, 3, 0.1, 0.2, 0.3)
        assert p.x == 1.0
        assert p.y == 2.0
        assert p.z == 3.0
        assert p.roll == 0.1
    ...
    def test_angles_default_to_zero(self):
        p = Vect3DOriented(1, 2, 3)
        assert p.roll == 0.0
        assert p.pitch == 0.0
        assert p.yaw == 0.0


class TestVect3DOrientedPosition:
    def test_position_property(self):
        p = Vect3DOriented(1, 2, 3, 0.1, 0.2, 0.3)
        assert p.position == Vect3D(1, 2, 3)


class TestVect3DOrientedTransform:
    def test_identity(self):
        origin = Vect3DOriented(0, 0, 0)
        point = Vect3D(5, 3, 1)
        assert origin.transform(point) == point

    def test_translation_only(self):
        pose = Vect3DOriented(10, 20, 30)
        point = Vect3D(1, 2, 3)
        assert pose.transform(point) == Vect3D(11, 22, 33)

    def test_yaw_only_matches_2d(self):
        """With only yaw, 3D transform should match 2D."""
        pose_2d = Vect2DOriented(100, 50, math.pi / 4)
        pose_3d = Vect3DOriented(100, 50, 0, yaw=math.pi / 4)
...
class TestVect3DOrientedInverse:
    def test_inverse_roundtrip(self):
        pose = Vect3DOriented(10, 20, 30, 0.1, 0.2, 0.3)
        point = Vect3D(5, 3, 1)
        transformed = pose.transform(point)
        recovered = pose.inverse().transform(transformed)
...
class TestVect3DOrientedCompose:
    def test_compose_translations(self):
        a = Vect3DOriented(10, 20, 30)
        b = Vect3DOriented(5, 3, 1)
        c = a.compose(b)
        assert c == Vect3DOriented(15, 23, 31)

    def test_compose_with_yaw(self):
        """Matches 2D compose when only yaw is used."""
        robot_2d = Vect2DOriented(100, 0, math.pi / 2)
        sensor_2d = Vect2DOriented(0, 50, 0)
        result_2d = robot_2d.compose(sensor_2d)

        robot_3d = Vect3DOriented(100, 0, 0, yaw=math.pi / 2)
        sensor_3d = Vect3DOriented(0, 50, 0)
    ...
    def test_compose_with_pitch_only(self):
        ...
        parent = Vect3DOriented(0, 0, 0, pitch=math.pi / 2)
        child = Vect3DOriented(100, 0, 0)
    ...
    def test_compose_equivalence_with_transform(self):
        ...
        a = Vect3DOriented(10, 20, 30, 0.1, 0.2, 0.3)
        b = Vect3DOriented(5, 3, 1, 0.05, 0.1, 0.15)
        p = Vect3D(7, 11, 13)
...
class TestVect3DOrientedToDict:
    def test_basic(self):
        d = Vect3DOriented(1, 2, 3, 0.1, 0.2, 0.3).to_dict()
        assert d == {"x": 1.0, "y": 2.0, "z": 3.0, "roll": 0.1, "pitch": 0.2, "yaw": 0.3}

    def test_round_trip(self):
        p = Vect3DOriented(1, 2, 3, 0.1, 0.2, 0.3)
        assert Vect3DOriented.from_dict(p.to_dict()) == p


class TestVect3DOrientedFromDict:
    def test_full(self):
        p = Vect3DOriented.from_dict({"x": 1, "y": 2, "z": 3, "roll": 0.1, "pitch": 0.2, "yaw": 0.3})
        assert p == Vect3DOriented(1, 2, 3, 0.1, 0.2, 0.3)

    def test_angles_default(self):
        p = Vect3DOriented.from_dict({"x": 1, "y": 2, "z": 3})
        assert p.roll == 0.0
        assert p.pitch == 0.0
        assert p.yaw == 0.0


class TestVect3DOrientedConversion:
    def test_to_2d_oriented(self):
        p = Vect3DOriented(10, 20, 30, 0.1, 0.2, 0.3)
        p2 = p.to_2d_oriented()
        assert p2 == Vect2DOriented(10, 20, 0.3)


class TestVect3DOrientedRepr:
    def test_repr(self):
        p = Vect3DOriented(1, 2, 3)
        assert repr(p) == "Vect3DOriented(x=1.0, y=2.0, z=3.0, roll=0.0, pitch=0.0, yaw=0.0)"
