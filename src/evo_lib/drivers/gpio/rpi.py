...
class RpiGPIO(GPIO):
    ...
    def init(self) -> None:
        ...
        self._log.info("RpiGPIO '%s' initialized on pin %d (%s)", self.name, self._pin, self._direction)
    ...
    def close(self) -> None:
        ...
        if self._watch_thread is not None:
            ...
            if self._watch_thread.is_alive():
                self._log.warning("GPIO watch thread for pin %d did not stop in time", self._pin)
