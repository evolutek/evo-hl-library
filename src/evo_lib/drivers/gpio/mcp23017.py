...
class MCP23017Pin(GPIO):
    ...
    def read(self) -> Task[bool]:
        ...
        return ImmediateResultTask(self._hw_pin.value)
    ...
    def interrupt(self, edge: GPIOEdge = GPIOEdge.BOTH) -> Event[bool]:
        ...
        raise NotImplementedError("MCP23017 interrupt is not supported by the Adafruit driver")
...
class MCP23017Chip(ComponentHolder):
    ...
    def get_pin(
        ...
        gpio_pin = MCP23017Pin(name, self, pin, direction)
    ...
    def _get_i2c_pins(bus: int) -> tuple:
        ...
        raise ValueError(f"I2C bus {bus} not supported: board has no SCL{bus}/SDA{bus} pins")
