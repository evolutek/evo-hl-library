...
class Scheduler:
    ...
    def schedule_now(self, priority: int, action: Callable, args: tuple, kwargs: dict[str,]) -> sched.Event:
        self.s.enter(0, priority, action, argument = args, kwargs = kwargs)

    def schedule_after(self, delay: float, priority: int, action: Callable, args: tuple, kwargs: dict[str,]) -> sched.Event:
        self.s.enter(delay, priority, action, argument = args, kwargs = kwargs)

    def schedule_at(self, timepoint: float, priority: int, action: Callable, args: tuple, kwargs: dict[str,]) -> sched.Event:
